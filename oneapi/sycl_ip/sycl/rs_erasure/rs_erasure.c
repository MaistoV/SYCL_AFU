...
void rs_erasure (
	...
	printf("%s:%d: cell_length_cci_byte_width		: 0x%02x\n"	, __FILE__, __LINE__, rs_erasure_csr.cell_length_cci_byte_width		);
	...
	uint32_t    cell_length		= rs_erasure_csr.cell_length_cci_byte_width << CCI_BYTE_WIDTH_BITS ;
	
#ifdef DEBUG
	printf("%s:%d: master_read:\n", __FILE__, __LINE__);
	for ( unsigned int i = 0; i < cell_length*RS_K; i++ ) {
		printf("%02x ", ((uint8_t*)master_read)[i]);
		if ( ((i+1) % CCI_BYTE_WIDTH) == 0 ) {
			printf("\n");
		}
	}
	printf("\n");
	...
	assert( (cell_length % CCI_BYTE_WIDTH) == 0 ); // Must be an integer multiple
#endif
...
LOOP_CCI_LINES:
	...
	#define NUM_CCI_LINES	(cell_length / CCI_BYTE_WIDTH)
	for ( unsigned int cci_line_index = 0; cci_line_index < NUM_CCI_LINES; cci_line_index++ ) {
		// Array of k survived cell lines
		cci_line_t survived_cell_lines[RS_K];
...
LOOP_CELLS:
		...
		for ( unsigned int cell_index = 0; cell_index < RS_K; cell_index++ ){
			survived_cell_lines[cell_index] = master_read[ (cell_index * NUM_CCI_LINES) + cci_line_index ];
		}	

	#ifdef DEBUG
		for ( unsigned int cell_index = 0; cell_index < RS_K; cell_index++ ){
			printf("%s:%d: survived_cell_lines[%d] for cci_line_index=%d:\n", __FILE__, __LINE__, cell_index, cci_line_index);
			for ( int byte_index = 0; byte_index < CCI_BYTE_WIDTH; byte_index++ ) {
				printf("%02x ", ((uint8_t (*)[CCI_BYTE_WIDTH])survived_cell_lines)[cell_index][byte_index] );
			}
			printf("\n");
		}
		printf("\n");
		...
		cci_line_t reconstructed_cell_line;
		// #pragma unroll
		reconstructed_cell_line = (cci_line_t)0u;

		// Loop over bytes in a cell
LOOP_BYTES:
		#pragma unroll
		for ( unsigned int cell_byte_index = 0; cell_byte_index < CCI_BYTE_WIDTH; cell_byte_index++ ) {
...
LOOP_BITS:
				...
				for ( uint8_t bit_idx = 0; bit_idx < GF_ORDER; bit_idx++ ) {
						...
						uint8 survived_byte = ((uint8 (*)[CCI_BYTE_WIDTH])survived_cell_lines)[scratchpad_register_byte_idx][cell_byte_index]; 
		...
		/////////////////////////////////////////
		master_write[ cci_line_index ] = reconstructed_cell_line;

	#ifdef DEBUG
		printf("%s:%d: Output data on cci_line_index %d: \n", __FILE__, __LINE__, cci_line_index );
