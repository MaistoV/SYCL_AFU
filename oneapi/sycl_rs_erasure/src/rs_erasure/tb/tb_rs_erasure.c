...
int main(int argc, char *argv[]) {
	...
	int encode_isal = 0;
	int decode_isal = 0;
	int cell_length = CELL_LENGTH_DEFAULT;
	...
	if ( encode_isal ) {
		...
		for ( unsigned int i = 0; i < RS_P; i++ ){
			...
			// Call to component
			ret_val = rs_erasure( master_write, master_read, rs_erasure_csr );
	...
	for ( unsigned int permutation_index = 0; permutation_index < max_permutations; permutation_index++ ) {
		...
		for ( unsigned int survival_index = 0; survival_index < num_vectors_per_erasure_pattern; survival_index++ ) {
			...
			if ( decode_isal ) {
				...
				// }
			#ifdef DEBUG	
				print_matrix_2d(stdout, NUM_ERRORS, RS_K, (uint8_t*)(decode_matrix_hls[permutation_index]), "decode_matrix_hls ");
				print_matrix_2d(stdout, 1, RS_K, (uint8_t*)decode_index_hls[permutation_index][survival_index], "decode_index_hls[permutation_index]");
			#endif
		...
		}  // survival_index, num_vectors_per_erasure_pattern
	} // permutation_index, max_permutations
