...
// Utility function wrapping the complexity of the SYCL_GPU call
void RunKernel(
		int measure_latency,
		FILE* fd_latency,
...
int main(int argc, char *argv[]) {
	...
	printf("%s:%d: Encoding parity cells for RS[%d:%d] cell_length=%d, using %s\n",
		 __FILE__, __LINE__, RS_K, RS_P, cell_length, (encode_isal) ? "ISA-L" : "SYCL_GPU kernel");
	...
	// 	ec_encode_data(cell_length, RS_K, RS_P, g_tbls, cell_ptrs, &(cell_ptrs[RS_K]));
        printf("[WARNING] Skipping calls to ISA-L!\n");
	} // encode_isal
	...
	// Debug rs_erasure_input
		...
		for ( unsigned int e = 0; e < RS_P; e++ ){
			printf("%s:%d: Encoding parity cell %d [%d/%d] with SYCL_GPU kernel\n", __FILE__, __LINE__, e, e+1, RS_P);
...
// Debug Complete cell array
	...
	printf("%s:%d: Decoding/Reconstructing blocks RS[%d:%d] cell_length=%d, using %s\n",
		 __FILE__, __LINE__, RS_K, RS_P, cell_length, (decode_isal) ? "ISA-L" : "SYCL_GPU kernel");
    ...
    if ( measure_latency ) {
		...
		// Open output file
		sprintf( filename, "%s_%d_%d_%d%s_%s.txt", BASE_FILENAME, RS_K, RS_P, cell_length_byte, cell_length_byte_power, (decode_isal) ? "ISA-L" : "SYCL_GPU");
	...
	for ( unsigned int permutation_index = 0; permutation_index < max_permutations; permutation_index++ ) {
		for ( unsigned int survival_index = 0; 
			...
			// 	ret_val = memcmp(recover_outp[i], cell_ptrs[permutation_index], cell_length);
			// 	if ( ret_val ) {
	...
	printf("%s:%d: Test passed\n RS[%d:%d]\n cell_length=%d,\n encodind with %s,\n decoding with %s,\n PRNG seed=%u\n",
		 ...
		 (encode_isal) ? "ISA-L" : "SYCL_GPU kernel",
		 (decode_isal) ? "ISA-L" : "SYCL_GPU kernel",
		 prng_seed
		 );
...
void RunKernel(
	...
	try {
		...
		// If the device is unavailable, a SYCL_GPU runtime exception is thrown.
		queue q(selector);
		...
		std::cerr << "Caught a SYCL_GPU host exception:\n" << e.what() << "\n";
