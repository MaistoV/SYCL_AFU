...
// #include <sycl/ext/intel/ac_types/ac_int.hpp> // FPGA-only
...
#define LOOP_READ_CELLS_UNROLL_HEURISTIC (RS_K / 2)
#ifndef LOOP_READ_CELLS_UNROLL
  #if (RS_K > LOOP_READ_CELLS_UNROLL_THRESHOLD)
    // If heuristic is not greater than threshold
    #if (LOOP_READ_CELLS_UNROLL_HEURISTIC < LOOP_READ_CELLS_UNROLL_THRESHOLD)
      #define LOOP_READ_CELLS_UNROLL LOOP_READ_CELLS_UNROLL_HEURISTIC
    #else
      // Limit to LOOP_READ_CELLS_UNROLL_THRESHOLD
